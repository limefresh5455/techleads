...
import { Home, Users, Globe, LinkIcon, Folder, MessageSquare, CreditCard, DollarSign, UserCheck, ShieldAlert } from 'lucide-react'
    ...
    <div className="space-y-6 max-w-7xl mx-auto w-full">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
        <div className="bg-surface border border-border rounded-xl p-5 shadow-sm">
          ...
          <div className="text-3xl font-extrabold text-ink">{stats.total_users.toLocaleString()}</div>
        </div>
        <div className="bg-surface border border-border rounded-xl p-5 shadow-sm">
          <div className="flex items-center gap-3 mb-2 text-muted">
            <UserCheck size={18} />
          ...
          <div className="text-3xl font-extrabold text-ink">{stats.total_customers.toLocaleString()}</div>
        </div>
        <div className="bg-surface border border-border rounded-xl p-5 shadow-sm">
          <div className="flex items-center gap-3 mb-2 text-muted">
            <ShieldAlert size={18} />
          ...
          <div className="text-3xl font-extrabold text-ink">{stats.total_admins.toLocaleString()}</div>
        </div>
        <div className="bg-surface border border-border rounded-xl p-5 shadow-sm">
          <div className="flex items-center gap-3 mb-2 text-muted">
            <CreditCard size={18} />
          ...
          <div className="text-3xl font-extrabold text-ink">{stats.active_plans.toLocaleString()}</div>
        </div>
        <div className="bg-surface border border-border rounded-xl p-5 shadow-sm">
          <div className="flex items-center gap-3 mb-2 text-muted">
            <DollarSign size={18} />
          ...
          <div className="text-3xl font-extrabold text-ink">{formatCurrency(stats.total_revenue)}</div>
        </div>
        <div className="bg-surface border border-border rounded-xl p-5 shadow-sm">
          <div className="flex items-center gap-3 mb-2 text-muted">
            <LinkIcon size={18} />
          ...
          <div className="text-3xl font-extrabold text-ink">{stats.total_websites.toLocaleString()}</div>
        </div>
        <div className="bg-surface border border-border rounded-xl p-5 shadow-sm">
          <div className="flex items-center gap-3 mb-2 text-muted">
            <Globe size={18} />
          ...
          <div className="text-3xl font-extrabold text-ink">{stats.total_technologies.toLocaleString()}</div>
        </div>
        <div className="bg-surface border border-border rounded-xl p-5 shadow-sm">
          <div className="flex items-center gap-3 mb-2 text-muted">
            <Folder size={18} />
          ...
          <div className="text-3xl font-extrabold text-ink">{stats.total_categories.toLocaleString()}</div>
        </div>
        <div className="bg-surface border border-border rounded-xl p-5 shadow-sm">
          <div className="flex items-center gap-3 mb-2 text-muted">
          ...
          <div className="text-3xl font-extrabold text-ink">{stats.total_messages.toLocaleString()}</div>
        </div>
      </div>
          ...
          <div className="w-full h-72">
            <ResponsiveContainer width="100%" height="100%">
              <AreaChart data={stats.revenue_graph}>
                ...
                />
                <Area type="monotone" dataKey="revenue" stroke="#FF6B35" strokeWidth={3} activeDot={{r: 6}} fillOpacity={1} fill="url(#colorRevenue)" />
              </AreaChart>
            </ResponsiveContainer>
          </div>
        </div>
            ...
            <table className="w-full text-left text-sm text-ink">
              ...
              <tbody className="divide-y divide-border">
                ...
                {stats.recent_signups.length === 0 && (
                  <tr>
                    <td colSpan={3} className="px-4 py-6 text-center text-muted">No recent signups</td>
                  </tr>
                )}
              </tbody>
            </table>
              ...
              <tbody className="divide-y divide-border">
                {stats.recent_messages.map((msg, idx) => (
                  <tr key={idx} className="hover:bg-card/50">
                    ...
                    <td className="px-4 py-3 text-xs">{new Date(msg.created_at).toLocaleDateString()}</td>
                  </tr>
                ))}
                {stats.recent_messages.length === 0 && (
                  <tr>
                    <td colSpan={3} className="px-4 py-6 text-center text-muted">No recent messages</td>
                  </tr>
                )}
              </tbody>
            </table>
