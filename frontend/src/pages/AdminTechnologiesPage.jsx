...
export default function AdminTechnologiesPage() {
  ...
  const [formData, setFormData] = useState({ 
    ...
    category_id: '', is_featured: true, is_popular: false, sort_order: 0, website_count: 0
  })
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left text-sm">
            ...
            <tbody className="divide-y divide-border">
                ...
                return (
                  <tr key={item.id} className="hover:bg-canvas/50 transition-colors">
                    <td className="px-6 py-4 text-ink font-medium flex items-center gap-2">
                      <span className="w-3 h-3 rounded-full" style={{ backgroundColor: item.icon_color || '#FF6B35' }}></span>
                      {item.name}
                    </td>
              ...
              {items.length === 0 && (
                <tr>
                  <td colSpan="4" className="p-8 text-center text-muted">No Technologies found.</td>
                </tr>
              )}
            </tbody>
          </table>
