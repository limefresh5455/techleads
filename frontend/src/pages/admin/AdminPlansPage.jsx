...
export default function AdminPlansPage() {
  ...
  const handleOpenModal = (plan = null) => {
    ...
    setFormData(
      plan
        ? { ...plan, features: plan.features ? JSON.parse(JSON.stringify(plan.features)) : [] }
        : defaultForm
    )
    setIsModalOpen(true)
  }
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-canvas border-b border-border">
                ...
                <th className="p-4 text-xs font-semibold text-muted uppercase tracking-wider text-center">
                  Popular
                </th>
                <th className="p-4 text-xs font-semibold text-muted uppercase tracking-wider w-32 text-right">
                  Actions
                </th>
              </tr>
            </thead>
            ...
            <div className="overflow-y-auto p-6">
              <form id="planForm" onSubmit={handleSave} className="space-y-6">
                ...
                <div className="space-y-4">
                  <h3 className="text-sm font-semibold uppercase text-muted tracking-wider">
                    Basic Details
                  </h3>
                  ...
                  <div className="grid grid-cols-3 gap-4">
                    <div>
                      ...
                      <input
                        ...
                        onChange={(e) =>
                          setFormData({ ...formData, monthly_price: parseInt(e.target.value) || 0 })
                        }
                        className="w-full bg-canvas border border-border rounded-lg px-3 py-2 text-ink focus:outline-none focus:border-brand"
                      />
                    </div>
                    <div>
                      ...
                      <input
                        ...
                        onChange={(e) =>
                          setFormData({ ...formData, yearly_price: parseInt(e.target.value) || 0 })
                        }
                        className="w-full bg-canvas border border-border rounded-lg px-3 py-2 text-ink focus:outline-none focus:border-brand"
                      />
                    </div>
                  ...
                  {formData.features.length === 0 ? (
                    ...
                    <div className="space-y-2">
                      {formData.features.map((feature, index) => (
                        <div
                          key={index}
                          className="flex gap-3 items-start bg-canvas p-3 rounded-lg border border-border"
                        >
                          <label
                            className="flex items-center pt-2 cursor-pointer"
                            title="Is this feature included?"
                          >
                            <input
                              type="checkbox"
