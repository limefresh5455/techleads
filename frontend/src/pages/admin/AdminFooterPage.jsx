    ...
    <div className="space-y-6 max-w-7xl mx-auto w-full">
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 pb-6 border-b border-border">
        <h1 className="text-2xl font-bold text-ink flex items-center gap-3">
          <LayoutTemplate className="text-brand-dark" />
          Footer Settings
        </h1>
        <button
            ...
            <tbody className="divide-y divide-border">
              {columns.map((col) => (
                <tr key={col.id} className="hover:bg-canvas/50">
                    ...
                    {col.links && col.links.length > 0 && (
                      <div className="mt-2 flex flex-wrap gap-2">
                        {col.links.map((link) => (
                          <span
                            key={link.id}
                            className="text-xs bg-canvas px-2 py-1 rounded text-ink border border-border"
                          >
                            {link.label}
                          </span>
                        ))}
                  ...
                  <td className="p-4 text-muted">{col.links?.length || 0} links</td>
          ...
          <div className="bg-surface rounded-xl shadow-xl w-full max-w-2xl border border-border flex flex-col max-h-[90vh]">
            <div className="flex items-center justify-between px-6 py-4 border-b border-border shrink-0">
              <h2 className="text-lg font-bold text-ink flex items-center gap-2">
                <LayoutTemplate className="text-brand-dark" size={24} />
