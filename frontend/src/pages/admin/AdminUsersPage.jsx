            ...
            <div className="p-6 overflow-y-auto">
              ...
              {editingItem && editingItem.credit_purchases && formData.role !== 'admin' && (
                <div>
                  ...
                  <div className="bg-canvas rounded-lg border border-border overflow-x-auto">
                    <table className="w-full text-left text-sm text-ink">
                        ...
                        {editingItem.credit_purchases.map((cp) => (
                          <tr key={cp.id}>
                            ...
                            <td className="px-4 py-3">
                              <span
                                className={`px-2 py-0.5 rounded text-xs font-medium ${cp.status === 'paid' ? 'bg-green-500/20 text-green-500' : 'bg-yellow-500/20 text-yellow-500'}`}
                              >
                                {cp.status}
                              </span>
                            </td>
                            <td className="px-4 py-3 text-ink/60">{formatDate(cp.created_at)}</td>
                          </tr>
                        ))}
