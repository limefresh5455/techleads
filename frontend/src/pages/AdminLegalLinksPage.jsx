...
import { adminLegalLinks } from '../adminApi'
...
export default function AdminLegalLinksPage() {
  ...
  const loadLinks = async () => {
    try {
      const data = await adminLegalLinks.getAll()
      setLinks(data || [])
    } catch (err) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (isEditing) {
        await adminLegalLinks.update(currentId, { label: formData.label })
      } else {
        await adminLegalLinks.create(formData)
      }
      await loadLinks()
      setIsModalOpen(false)
    } catch (err) {
      console.error('Failed to save legal link:', err)
      alert('Error saving legal link.')
    } finally {
      setSaving(false)
    }
  }
  ...
  const handleDelete = async (id) => {
    if (!window.confirm('Are you sure you want to delete this legal link?')) return
    try {
      await adminLegalLinks.delete(id)
      await loadLinks()
    } catch (err) {
      console.error('Failed to delete legal link:', err)
      alert('Error deleting legal link.')
    }
  }
    ...
    <div className="space-y-6 max-w-7xl mx-auto w-full">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-6 border-b border-border">
        <h1 className="text-2xl font-bold text-ink flex items-center gap-3">
          <Shield className="text-brand-dark shrink-0" />
          <span className="whitespace-nowrap">Legal Links</span>
        </h1>
        <button
          ...
          <div className="bg-surface w-full max-w-md rounded-2xl shadow-xl border border-border overflow-hidden">
            <div className="flex items-center justify-between p-6 border-b border-border">
              <h2 className="text-xl font-bold text-ink flex items-center gap-2">
                ...
                {isEditing ? 'Edit Legal Link' : 'New Legal Link'}
              </h2>
              <button
