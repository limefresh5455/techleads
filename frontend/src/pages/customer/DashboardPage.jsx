...
import { SiteDetailsPanel } from '../../components'
import { CreditsPanel } from '../../components'
...
export default function DashboardPage() {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      try {
        ...
        const rows = await fetchTechnologies(q, 0, 50)
        if (!cancelled) {
          setRemoteTechs(rows)
  ...
  const loadMoreTechs = useCallback(async () => {
    ...
    try {
      const q = techSearch.trim()
      const rows = await fetchTechnologies(q, techOffset, 50)
      setRemoteTechs((prev) => {
  ...
  }, [techSearch, techOffset, isFetchingTechs, hasMoreTechs])

  const filteredTechnologies = useMemo(() => {
    const list = [...remoteTechs]
    const selectedObjs = selectedTechs
      .map((slug) => list.find((t) => t.slug === slug) || technologies.find((t) => t.slug === slug))
      .filter(Boolean)
  ...
  async function exportCsv() {
    ...
    if (selectedTechs.length > 0) {
      ...
      if ((results.user_credits ?? 0) < cost) {
        setError(
          `Need ${cost} credit${cost === 1 ? '' : 's'} to export ${cost} technolog${cost === 1 ? 'y' : 'ies'}.`
        )
        return
      }
    }
        ...
        <aside className="self-start rounded-2xl border border-border bg-card p-4 shadow-sm">
          ...
          <div className="mt-5">
            ...
            <div className="mt-3 flex flex-wrap gap-2">
              {(domainQuery || techSearch || selectedTechs.length > 0) && (
                <button
                  type="button"
                  onClick={() => {
                    setDomainQuery('')
